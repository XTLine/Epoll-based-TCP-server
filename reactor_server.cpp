...
#include "include/reactor/reactor_server.h"
...
int init_reactor(struct net_reactor *reactor)
{
    if(reactor ==NULL) 
        return REACTOR_NULL;
    memset (reactor,0,sizeof(struct net_reactor)); //对reactor初始化
    ...
    if(items == NULL)
    {
        printf("create socket_item in %s error %s \n",__func__,strerror(errno));//：__func__ 常用于打印日志或输出调试信息时，以标识当前代码所在的函数。
        close(reactor->epollfd);
        return REACTOR_MALLOC_MEMORY_FAILED;
    }
    memset(items,0,MAX_SOCKET_ITEMS*sizeof(struct socket_item));//对item初始化
    //创建事件内存块
...
int init_socket(short port)
{
    ...
    //AF_INET：指定了使用 IPv4 地址族，表示该套接字将用于 IPv4 网络通信。
    //SOCK_STREAM：指定了套接字的类型为流套接字，即 TCP 套接字，提供可靠的、面向连接的数据传输服务。
    //0：表示默认协议，根据套接字类型（SOCK_STREAM）和地址族（AF_INET）自动选择合适的协议，通常是 TCP 协议。
    if(fd==-1)
    {
    ...
    //参数：
    // fd：是要操作的文件描述符。
    // F_GETFL：是操作命令，表示获取文件状态标志。
    // 0：通常用于获取当前文件描述符的标志信息。
    flag |=O_NONBLOCK;
...
{
    if(reactor=NULL) return REACTOR_NULL;
    if(reactor->start_block==NULL) return REACTOR_NULL;
    //找到链表末端
